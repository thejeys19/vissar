  ...
  return (
    <div className={`p-4 min-h-full overflow-x-hidden w-full max-w-full flex flex-col ${containerBg}`}>
      ...
      {(layout === 'wall-sm' || layout === 'wall-md' || layout === 'wall-lg') && (() => {
        ...
        const animDuration = isLg ? 20 : isMd ? 18 : 16;

        // Distribute reviews across rows — each row gets a different slice so content varies
        const baseReviews = [...reviews, ...reviews, ...reviews];
        const rowSets = Array.from({ length: cfg.rows }, (_, rowIdx) => {
          // Offset each row by 1-2 reviews so rows show different content
          const offset = rowIdx * 2;
          return baseReviews.slice(offset, offset + Math.max(reviews.length, 4));
        });

        return (
          <div className="flex flex-col justify-center gap-2 w-full max-w-full flex-1" style={{ overflow: 'hidden' }}>
            {rowSets.map((rowReviews, rowIdx) => {
              // Alternate direction: even rows scroll left, odd rows scroll right
              const animName = rowIdx % 2 === 0 ? 'vissar-scroll-ltr' : 'vissar-scroll-rtl';
              return (
                <div key={rowIdx} style={{ overflow: 'hidden', width: '100%' }}>
                  ...
                  >
                    ...
                    {[...rowReviews, ...rowReviews].map((review, i) => (
                      <div
                        key={i}
                        style={{ width: `${cfg.cardW}px`, flexShrink: 0 }}
            ...
            <style>{`
              @keyframes vissar-scroll-ltr { from { transform: translateX(0); } to { transform: translateX(-50%); } }
              @keyframes vissar-scroll-rtl { from { transform: translateX(-50%); } to { transform: translateX(0); } }
            `}</style>
          </div>
        );
      })()}
